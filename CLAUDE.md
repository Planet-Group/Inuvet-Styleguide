...
2c. **Marken-Haut (Muster):** `brand-planimol.css` — gleicher Basename im Theme (`assets/` flach). Planimol = `html[data-brand="planimol"]` + diese Datei. Styleguide: Sidebar + `?brand=planimol`. Theme: Setting `brand` (Default `inuvet`) · Snippets `html-brand-attr` / `brand-skin`. Nicht das Repo `Planimol-Styleguide` weiterbauen. **Inhalte** (Benefits, Praxis, Lottie-JSONs) sind nicht die Haut — sie liegen pro Shopify-Store in markenneutralen Metaobjects `shop_benefit` / `shop_praxis`. Spec → Theme `CLAUDE.md` „Marke: ein Theme, zwei Kühlschränke“.
